...
class OneDimensionalMoveClass:
	def __init__(self, x0, a, v0=0, vmax=0):
		self._x0 = x0
		self._v0 = v0
		self._old_x = x0
		self._a = a
	...
	def get_x(self):
		...
		old_x=self._old_x
		if(tim>= self._tim0+self._t3):
			self._old_x=self._des_x
			return old_x,self._old_x
		elif tim>self._tim0+self._t2:
			t=tim-self._tim0-self._t2
			delta_x=self._s0 + self._s1 + self._current_vmax*t-0.5*self._current_a*t**2
			self._old_x=self._x0+delta_x
		elif tim > self._tim0 + self._t1:
...
od_move=OneDimensionalMoveClass(10,100)
